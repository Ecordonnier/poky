...
do_install () {
    ...
    mv ${D}/usr/include/openssl ${D}/usr/include/openssl-backported
    rm -rf ${D}/usr/share
    rm -rf ${D}/usr/bin
    rm ${D}/usr/lib/libcrypto.so
    rm ${D}/usr/lib/libcrypto.a
    rm ${D}/usr/lib/libssl.so
    rm ${D}/usr/lib/libssl.a
...
PACKAGES =+ "libcrypto-backported libssl-backported openssl-conf-backported ${PN}-engines"
# remove some files from -dev package to avoid conflict with the "openssl" package (the one which is not backported)
FILES:${PN}-dbg = "${libdir}/*.so /usr/include/openssl-backported/*.h ${libdir}/pkgconfig/*.pc"
FILES:libcrypto-backported = "${libdir}/libcrypto${SOLIBS}"
FILES:libssl-backported = "${libdir}/libssl${SOLIBS}"
FILES:openssl-conf-backported = "${sysconfdir}/ssl/openssl.cnf \
...
CONFFILES:openssl-conf-backported = "${sysconfdir}/ssl/openssl.cnf"

RRECOMMENDS:libcrypto-backported += "openssl-conf-backported"
RDEPENDS:${PN}-ptest += "openssl-backported-bin perl perl-modules bash"

RDEPENDS:${PN}-bin += "openssl-conf-backported"
